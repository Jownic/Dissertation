...
Find 50 AAA features in the provided manual and provide the commands for them.
Write them into a list format numbered 1 to 50 listed under each other
{context_snippets}
...
lines = full_cli.splitlines()
configs = [l.strip() for l in lines if re.match(r"^\d+\.\s", l)]

print(f"Found {len(configs)} items")
for cfg in configs:
    print(cfg)
