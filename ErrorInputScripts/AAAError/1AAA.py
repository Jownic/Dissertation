...
output_dir = r"Z:\Thesis\ErrorInputScripts\AAAError\1AAA"
os.makedirs(output_dir, exist_ok=True)

def remove_one_aaa_regex_block(config_text, seed):
    """
    Removes exactly one AAA-related block from the given config_text,
    chosen at random (based on seed) from the patterns that actually exist.
    Returns the modified text and a list containing the removed block.
    """
    random.seed(seed)
    # Find which patterns are present
    existing = [p for p in aaa_features if re.search(p, config_text, re.MULTILINE)]
    # Pick one at random if any exist
    to_remove = random.sample(existing, 1) if existing else []
    print(f"[DEBUG] Seed {seed} selected pattern to remove: {to_remove}")
...
# Process router1 through router5
for i in range(1, 6):
    ...
    modified, removed = remove_one_aaa_regex_block(original, seed=i)

    with open(output_path, 'w') as f:
        f.write(modified)

    if removed:
        print(f"[Router {i}] ✅ Removed block:")
