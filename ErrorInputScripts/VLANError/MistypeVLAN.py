...
output_dir = r"Z:\Thesis\ErrorInputScripts\VLANError\VLANMistype"
...
def mistype_one_vlan_feature(config_text, seed):
    """
    Finds all lines matching any VLAN feature pattern,
    ...
    # Find all VLAN feature lines
    matches = [
        (i, line) for i, line in enumerate(lines)
        if any(re.match(pat, line) for pat in vlan_feature_patterns)
    ]
    if not matches:
...
# Process router67 through router85
for i in range(67, 85):
    ...
    modified, orig_line, typo_line = mistype_one_vlan_feature(original, seed=i)
    ...
    if orig_line:
        ...
        print(f"[Router {i}] ⚠️ No VLAN feature lines found to mistype.")
